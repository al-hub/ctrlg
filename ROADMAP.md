# ctrlg 기능 개선 로드맵 (v0.3.1)

> 본 문서는 ctrlg 프로젝트의 기능 개선 로드맵 및 구현 상태 추적 문서입니다.

---

## 🎉 v0.3.1 완료 구현 성과 (Recent Accomplishments)

현재 다음 핵심 요구사항들이 성공적으로 구현되어 배포 및 검증(10/10 PASS) 완료되었습니다:

1. **FZF 기반 대화형 TUI 전향 (`src/interactive.sh`)**
   - 쉘 화면 꼬임 및 질의 삭제 오류를 완벽하게 방지하기 위해, 독자적인 FZF 대화형 세션을 구축하여 사용자 질의를 안전하게 가공합니다.
2. **질의 히스토리 선택창 및 AI 재전송 (`~/.config/ctrlg/history.log` 연동)**
   - `Ctrl + G`를 누르면 과거에 입력했던 질문(질의) 목록이 최신순으로 정렬되어 노출되며, 이를 검색/선택하여 AI에 실시간 재전송 및 캐싱할 수 있습니다.
3. **쉘 에디터 화면 깨짐 원천 차단**
   - Zsh 환경에서 `zle -I` (Invalidate) API를 활용해 터미널 출력이 진행되는 동안 편집기를 안전하게 중단시킴으로써 원래 질의와 프롬프트 형상을 화면에 완벽히 보존합니다.
4. **에러 감지 훅 비활성화**
   - 명령어 에러 자동 가이드 훅(`ctrlg_error_hook`)의 비활성화를 적용하여 쉘 프롬프트 본연의 고속 반응성을 회복했습니다.
