# 🖥️ 로컬 LLM 기반 크로스플랫폼 터미널 AI 도우미 (ctrlg) 구축 계획서 - v0.3.0
...
graph TD
    ...
    B -->|Zsh/Bash Widget 작동| C[로컬 tldr RAG 컨텍스트 추출]
    C --> D[Ollama 2B~3B 초고속 추론]
    D --> E[명령어 쉘 입력창에 자동 주입]
    E --> F[사용자가 직접 수정 후 Enter 실행]
    
    G[일반 명령어 실패 Exit != 0] --> H[에러 훅 감지]
    H --> I[AI 에러 해결책 자동 제안]
```
...
### 🎯 철학 ① : 쉘 버퍼에 즉시 채워 넣기 (UX 개선)
* **목적**: `y/N` 확인 단계를 과감히 삭제하고, 생성된 명령어를 터미널 입력 줄(Buffer)에 바로 써주어 사용자가 직접 눈으로 검증하고 즉시 편집하여 실행할 수 있도록 합니다.

#### 🛠️ 구체적 구현 방법 (Zsh / Bash Widget 연동)
사용자가 터미널에 한국어로 설명(예: `현재 폴더에서 로그 파일 찾기`)을 적고 단축키 **`Ctrl + G`**를 누르면, AI가 작동하여 해당 한글을 지우고 최적의 쉘 명령어를 그 자리에 타이핑해 주는 쉘 에디터 위젯 구조입니다.

* **Zsh (`~/.zshrc`) 구현 방법**:
  ```zsh
  # Zsh Line Editor (ZLE)용 위젯 함수
  ctrlg-widget() {
      # 1. 현재 입력창에 쓰여 있는 텍스트 추출
      local query="$BUFFER"
      if [ -z "$query" ]; then
          return
      fi
      ...
      # 3. 백그라운드 ctrlg 스크립트 실행하여 순수 CMD만 획득
      local result=$(ctrlg --raw "$query")

      # 4. 입력창을 AI가 제안한 명령어 복사본으로 대체
      BUFFER="$result"
      CURSOR=$#BUFFER      # 커서를 줄 맨 끝으로 이동
      POSTDISPLAY=""       # 로딩 안내 지우기
      zle redisplay
  }
